export const articles = [
  {
    id: "12345",
    category: "生活",
    title: "退休後才明白，真正靠得住的不是子女",
    excerpt:
      "很多人年輕時拼命工作，把最好的都留給孩子，直到退休後才發現，真正能陪伴自己走到最後的，往往是健康與獨立。",
    content: `
很多人年輕時拼命工作，
退休後才發現真正能陪伴自己的，
往往不是想像中的那群人……

當年我們努力賺錢，
為孩子買房買車，
最後卻發現……

人到晚年，
最重要的是健康與獨立。
    `,
    views: "12.5K",
    date: "2026-06-03",
    image: "/images/article-1.jpg"
  },

  {
    id: "12346",
    category: "lifestyle",
    title: "妻子偷偷轉走存款後，我終於醒悟",
    excerpt: "結婚二十年，我一直以為信任是婚姻最堅固的基礎，直到那天發現存款被悄悄轉走，才明白有些事情再也回不去了。",
    content: "第二篇文章內容...",
    views: "8.9K",
    date: "2026-06-03",
    image: "https://picsum.photos/1200/700?2"
  },

  {
    id: "12347",
    category: "health",
    title: "醫生提醒：這3種早餐別天天吃",
    excerpt: "不少人以為每天吃的早餐很健康，其實有些常見食物長期食用可能增加身體負擔，醫生提醒要特別留意。",
    content: "第三篇文章內容...",
    views: "21.3K",
    date: "2026-06-02",
    image: "https://picsum.photos/1200/700?3"
  },

  {
  id: "12348",
  category: "lifestyle",
  title: "60歲後才明白，存款比面子重要",
  excerpt: "年輕時總想活給別人看，退休後才發現真正重要的是自己……",
  content: `
很多人一輩子都在追求別人的認同。

買大房子、開好車、請客吃飯。

直到退休後才發現，
真正能帶來安全感的，
其實是穩定的存款與健康的身體。
  `,
  views: "15.2K",
  date: "2026-06-05",
  image: "https://picsum.photos/1200/700?4" 
},

{
  id: "12349",
  category: "relationships",
  title: "老同學聚會後，我刪掉了所有聯絡方式",
  excerpt:
    "原以為多年不見會格外親切，沒想到一場聚會讓我看清了許多人情冷暖……",
  content: `
多年未見的老同學再次聚在一起。

大家聊工作、聊收入、聊孩子。

原本期待的重逢，
卻慢慢變成了比較與炫耀。

回家後我想了很久。

真正值得珍惜的朋友，
不需要靠聚會證明感情。

有些關係停留在回憶裡，
反而更美好。
  `,
  views: "10.8K",
  date: "2026-06-05",
  image: "https://picsum.photos/1200/700?5",
},

{
  id: "12350",
  category: "lifestyle",
  title: "退休金只有20萬，我卻活得比以前快樂",
  excerpt:
    "很多人擔心退休後沒錢，其實真正決定幸福感的，未必只是存款數字……",
  content: `
退休後的第一年，
我每天都在擔心錢不夠用。

後來我開始種花、
散步、
閱讀。

我發現生活需要的東西其實不多。

少了工作的壓力，
反而擁有了更多自由。

幸福從來不是擁有多少，
而是懂得知足。
  `,
  views: "14.2K",
  date: "2026-06-05",
  image: "https://picsum.photos/1200/700?6",
},

{
  id: "12351",
  category: "health",
  title: "醫生提醒：睡前這個習慣可能影響健康",
  excerpt:
    "不少人每天都在做，卻不知道長期下來可能影響睡眠品質……",
  content: `
許多人睡前喜歡滑手機。

藍光會影響褪黑激素分泌，
讓大腦保持清醒。

久而久之，
容易出現失眠與疲勞問題。

睡前一小時放下手機，
往往能改善睡眠品質。
  `,
  views: "25.1K",
  date: "2026-06-04",
  image: "https://picsum.photos/1200/700?7",
},

{
  id: "12352",
  category: "relationships",
  title: "女兒出嫁那天，我在婚禮上偷偷哭了",
  excerpt:
    "養育二十多年的孩子終於成家，那一刻才明白父母心中的不捨……",
  content: `
從小抱在懷裡的孩子，
轉眼已經穿上婚紗。

婚禮上大家都在笑，
只有我偷偷紅了眼眶。

不是難過，
而是感動。

看著她找到幸福，
一切付出都值得了。
  `,
  views: "18.7K",
  date: "2026-06-04",
  image: "https://picsum.photos/1200/700?8",
},

{
  id: "12353",
  category: "food",
  title: "超市員工透露：這些商品最好別太晚買",
  excerpt:
    "很多消費者不知道，其實某些商品在特定時間購買會更新鮮……",
  content: `
超市每天都會補貨。

部分熟食與生鮮產品，
通常在上午完成更新。

若太晚購買，
選擇可能變少。

養成查看日期與保存期限的習慣，
才能買得更安心。
  `,
  views: "16.4K",
  date: "2026-06-04",
  image: "https://picsum.photos/1200/700?9",
},

{
  id: "12354",
  category: "health",
  title: "60歲後才懂：健康才是真正的財富",
  excerpt:
    "年輕時拼命追求名利，到了晚年才發現健康的重要遠超過想像……",
  content: `
年輕時總以為時間很多。

為了工作熬夜、
應酬、
忽略身體。

直到年紀漸長，
才發現健康才是一切的基礎。

沒有健康，
再多財富也難以享受。

照顧好自己，
永遠不嫌晚。
  `,
  views: "19.9K",
  date: "2026-06-03",
  image: "https://picsum.photos/1200/700?10",
},

{
  id: "20001",
  category: "taiwan",
  title: "台灣這座小鎮爆紅，吸引大量遊客朝聖",
  excerpt: "原本低調的小鎮，最近因為獨特景色在網路上掀起熱議……",
  content: `
近年來不少台灣小鎮因網路分享而爆紅。

許多旅客慕名前往，
帶動當地觀光發展。

在地居民表示，
希望能保留原有特色，
同時迎接更多遊客。
  `,
  views: "6.8K",
  date: "2026-06-08",
  image: "https://picsum.photos/1200/700?11",
},

{
  id: "20002",
  category: "entertainment",
  title: "知名藝人罕見分享近況，引發粉絲關注",
  excerpt: "多年來鮮少公開露面的他，近日終於更新動態……",
  content: `
這位藝人近年相當低調。

近日透過社群平台分享生活近況，
吸引大量粉絲留言關心。

不少網友表示，
看到熟悉的身影感到相當開心。
  `,
  views: "12.3K",
  date: "2026-06-08",
  image: "https://picsum.photos/1200/700?12",
},

{
  id: "20003",
  category: "fortune",
  title: "命理師提醒：下半年運勢較旺的生肖",
  excerpt: "不少民眾關心運勢變化，專家提出觀察與建議……",
  content: `
每到新階段，
許多人都會關心自己的運勢。

命理師指出，
保持正向態度與良好習慣，
往往比運勢本身更重要。

努力與機會並存，
才能創造更好的未來。
  `,
  views: "9.1K",
  date: "2026-06-08",
  image: "https://picsum.photos/1200/700?13",
},

{
  id: "20004",
  category: "oddities",
  title: "男子整理老屋時，意外發現神秘收藏",
  excerpt: "原本不起眼的角落，竟藏著令人意想不到的物品……",
  content: `
一名男子在整理祖屋時，
意外發現一批保存完整的舊物。

消息曝光後，
引起許多收藏愛好者討論。

專家表示，
這些物品具有一定歷史價值。
  `,
  views: "14.7K",
  date: "2026-06-08",
  image: "https://picsum.photos/1200/700?14",
},

{
  id: "20005",
  category: "pets",
  title: "貓咪每天做這個動作，原來是在表達愛意",
  excerpt: "許多飼主每天都看到，卻不知道背後的真正含義……",
  content: `
不少貓咪會用頭輕輕磨蹭主人。

這其實是一種表達信任與親近的方式。

透過日常互動，
主人也能更了解毛孩的情緒。

陪伴與耐心，
是建立感情的重要關鍵。
  `,
  views: "18.2K",
  date: "2026-06-08",
  image: "https://picsum.photos/1200/700?15",
},

];